...
- [ ] Verify installation: `pnpm ls vitest`
...
- [ ] Verify: `pnpm run test` passes with zero tests
...
- [ ] Steps: checkout → setup Node 22.x → cache → `pnpm install --frozen-lockfile` → `pnpm run lint` → `pnpm run test` → `pnpm run build`
...
- [ ] Create `.husky/pre-push` with `pnpm run build`
