...
describe('matchLaptopClass — Scoring', () => {
  ...
  test.todo('no fingerprint-reader models in catalog yet — cannot assert count > 0')
})

describe('matchLaptopClass — Edge cases', () => {
  it('returns empty array when specs are mutually exclusive (no generic fallback)', () => {
    // budget: low + RTX + i7 + 16GB is intentionally contradictory — no model can match
    const specs = {
      ...baseSpecs,
      processor: 'Intel i7 / Ryzen 7',
      ram: '16 GB',
    ...
    const results = matchLaptopClass(specs)
    // matchLaptopClass has no generic fallback — returns empty for impossible constraints
